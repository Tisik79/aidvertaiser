...
function RelatedCard({ item }: { item: UseCase }) {
  const { i18n } = useTranslation();
  const lang = i18n.language?.slice(0, 2) || 'en';
  return (
    <Link to={`/use-cases/${item.slug}`} className="group relative flex flex-col rounded-2xl border border-zinc-200/50 dark:border-white/5 bg-white/50 dark:bg-white/[0.02] backdrop-blur-xl p-6 overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-indigo-500/5 hover:border-indigo-500/20 dark:hover:border-indigo-500/20">
...
function NotFound() {
  return (
    <section className="py-32"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
      ...
      <h1 className="text-2xl font-display font-bold text-zinc-900 dark:text-white mb-2">Use Case Not Found</h1>
      <p className="text-zinc-500 dark:text-zinc-400 mb-8">The use case you are looking for does not exist or has been moved.</p>
      <Link to="/use-cases" className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-zinc-900 dark:bg-white text-white dark:text-black text-sm font-semibold hover:scale-105 transition-transform"><ArrowLeft size={16} weight="bold" /> Back to Use Cases</Link>
    </div></section>
  );
}
...
export default function UseCaseDetailPage() {
  ...
  const { i18n } = useTranslation();
  const lang = i18n.language?.slice(0, 2) || 'en';
  const item = useCases.find((uc) => uc.slug === slug);
  const title = item ? item.title[lang] || item.title.en : '';
  const content = item ? item.content[lang] || item.content.en : '';
  const toc = useMemo(() => extractToc(content), [content]);
  const related = useMemo(() => useCases.filter((uc) => uc.slug !== slug).slice(0, 3), [slug]);
  ...
  const canonical = `${SITE_URL}/use-cases/${item.slug}`;
  const graphJsonLd = buildGraphJsonLd([webPageEntity(canonical, `${item.title.en} - Aidvertaiser`, item.shortDescription.en)]);

  return (
    <>
      <SEO title={`${title} - Aidvertaiser`} description={item.shortDescription[lang] || item.shortDescription.en} canonical={canonical} breadcrumbs={[{ name: 'Home', url: SITE_URL }, { name: 'Use Cases', url: `${SITE_URL}/use-cases` }, { name: title, url: canonical }]} jsonLd={graphJsonLd} />
      <section className="pt-12 pb-8 md:pt-20 md:pb-12"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <Breadcrumbs items={[{ label: 'Use Cases', href: '/use-cases' }, { label: title }]} />
        ...
        <section className="pb-8 md:pb-12"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div ref={featuresRef} className="p-6 rounded-2xl border border-zinc-200/50 dark:border-white/5 bg-white/50 dark:bg-white/[0.02]">
            ...
            <div className="flex flex-wrap gap-2">
              {item.platforms.map((p) => <span key={p} data-feature-item className="px-3 py-1.5 rounded-full bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 text-sm font-medium opacity-0">{p}</span>)}
            </div>
          </div>
        </div></section>
      )}
      ...
      {related.length > 0 && (
        <section className="py-16 md:py-24 border-t border-zinc-200/50 dark:border-white/5"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl md:text-3xl font-display font-bold tracking-tight text-zinc-900 dark:text-white mb-8">Other Use Cases</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{related.map((uc) => <RelatedCard key={uc.slug} item={uc} />)}</div>
        </div></section>
      )}
      <section className="py-16 md:py-24 bg-zinc-900 dark:bg-zinc-950 text-white"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          ...
          <Link to="/use-cases" className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full border border-white/10 text-zinc-300 text-sm font-semibold hover:bg-white/5 transition-colors"><ArrowLeft size={16} weight="bold" /> All Use Cases</Link>
        </div>
      </div></section>
    </>
  );
