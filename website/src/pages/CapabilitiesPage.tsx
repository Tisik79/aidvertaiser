...
function CapabilityCard({ capability }: { capability: Capability }) {
  ...
  const title = capability.title[lang] || capability.title.en;
  const description = capability.shortDescription[lang] || capability.shortDescription.en;

  return (
    <Link to={`/capabilities/${capability.slug}`} className="group relative flex flex-col rounded-2xl border border-zinc-200/50 dark:border-white/5 bg-white/50 dark:bg-white/[0.02] backdrop-blur-xl p-6 overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-indigo-500/5 hover:border-indigo-500/20 dark:hover:border-indigo-500/20">
      ...
      <div className="relative flex flex-col flex-1">
        <div className="flex items-start justify-between mb-4">
          <div className="w-10 h-10 rounded-lg bg-indigo-50 dark:bg-indigo-500/10 flex items-center justify-center text-indigo-600 dark:text-indigo-400 group-hover:scale-110 transition-transform text-xl">{capability.icon}</div>
        </div>
        ...
        {capability.platforms && capability.platforms.length > 0 && (
          <div className="mt-4 flex flex-wrap gap-1.5">
            {capability.platforms.slice(0, 3).map((p) => <span key={p} className="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-medium bg-zinc-100 dark:bg-white/5 text-zinc-500 dark:text-zinc-500">{p}</span>)}
            {capability.platforms.length > 3 && <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-medium bg-zinc-100 dark:bg-white/5 text-zinc-500 dark:text-zinc-500">+{capability.platforms.length - 3}</span>}
          </div>
        )}
...
export default function CapabilitiesPage() {
  ...
  const jsonLd = buildGraphJsonLd([webPageEntity(`${SITE_URL}/capabilities`, 'Capabilities - Aidvertaiser', 'Explore the core capabilities of Aidvertaiser: campaign management, conversion tracking, audience targeting, analytics, SEO, and real-time monitoring.')]);
  ...
  const animateGrid = useCallback(() => {
    ...
    const cards = gridRef.current.querySelectorAll('[data-capability-card]');
  ...
  useEffect(() => {
    if (!gridRef.current) return;
    if (prefersReducedMotion()) { gridRef.current.querySelectorAll('[data-capability-card]').forEach((el) => { (el as HTMLElement).style.opacity = '1'; }); return; }
  ...
  return (
    <>
      <SEO title="Capabilities - Aidvertaiser" description="Explore the core capabilities of Aidvertaiser: campaign management, conversion tracking, audience targeting, analytics, SEO, and real-time monitoring." canonical={`${SITE_URL}/capabilities`} breadcrumbs={[{ name: 'Home', url: SITE_URL }, { name: 'Capabilities', url: `${SITE_URL}/capabilities` }]} jsonLd={jsonLd} />
      <section className="pt-12 pb-8 md:pt-20 md:pb-12"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <Breadcrumbs items={[{ label: t('nav.capabilities', 'Capabilities') }]} />
        <div ref={heroRef} className="max-w-3xl opacity-0">
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-display font-bold tracking-tight text-zinc-900 dark:text-white">{t('capabilities.title', 'Core')}{' '}<span className="bg-clip-text text-transparent bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500">{t('capabilities.titleGradient', 'Capabilities')}</span></h1>
          <p className="mt-4 text-lg text-zinc-600 dark:text-zinc-400 leading-relaxed">{t('capabilities.subtitle', 'Six powerful capability areas covering every aspect of advertising management, from campaign creation to performance optimization.')}</p>
        </div>
      </div></section>
      <section className="pb-20 md:pb-32"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {capabilities.map((c) => <div key={c.slug} data-capability-card className="opacity-0"><CapabilityCard capability={c} /></div>)}
        </div>
      </div></section>
      <section className="py-20 md:py-32 bg-zinc-900 dark:bg-zinc-950 text-white relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-100px' }} transition={{ duration: 0.6 }} className="text-center">
            <h2 className="text-3xl md:text-4xl font-display font-bold tracking-tight">Ready to Supercharge Your Ads?</h2>
            <p className="mt-4 text-lg text-zinc-400 max-w-xl mx-auto">180 tools, one install. Open source, forever free.</p>
