...
function PlatformCard({ platform }: { platform: Platform }) {
  ...
  const title = platform.title[lang] || platform.title.en;
  const description = platform.shortDescription[lang] || platform.shortDescription.en;
      ...
      <div className="relative flex flex-col flex-1">
        ...
        {platform.features.length > 0 && (
          <div className="mt-4 flex flex-wrap gap-1.5">
            {platform.features.slice(0, 3).map((f) => <span key={f} className="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-medium bg-zinc-100 dark:bg-white/5 text-zinc-500 dark:text-zinc-500">{f}</span>)}
            {platform.features.length > 3 && <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-medium bg-zinc-100 dark:bg-white/5 text-zinc-500 dark:text-zinc-500">+{platform.features.length - 3}</span>}
          </div>
        )}
        <div className="mt-4 pt-4 border-t border-zinc-200/50 dark:border-white/5 flex items-center gap-1.5 text-sm font-medium text-indigo-600 dark:text-indigo-400 opacity-0 group-hover:opacity-100 transition-opacity">
          Learn more <ArrowRight size={14} weight="bold" className="transition-transform group-hover:translate-x-0.5" />
        </div>
      </div>
    </Link>
  );
...
export default function PlatformsPage() {
  ...
  const jsonLd = buildGraphJsonLd([webPageEntity(`${SITE_URL}/platforms`, 'Supported Platforms - Aidvertaiser', 'Explore all 6 advertising platforms supported by Aidvertaiser: Google Ads, Meta Ads, GA4, Search Console, Matomo, and Bing.')]);
  ...
  useEffect(() => {
    if (!gridRef.current) return;
    if (prefersReducedMotion()) { gridRef.current.querySelectorAll('[data-platform-card]').forEach((el) => { (el as HTMLElement).style.opacity = '1'; }); return; }
  ...
  return (
    <>
      <SEO title="Supported Platforms - Aidvertaiser" description="Explore all 6 advertising platforms supported by Aidvertaiser: Google Ads, Meta Ads, GA4, Search Console, Matomo, and Bing Webmaster Tools." canonical={`${SITE_URL}/platforms`} breadcrumbs={[{ name: 'Home', url: SITE_URL }, { name: 'Platforms', url: `${SITE_URL}/platforms` }]} jsonLd={jsonLd} />

      <section className="pt-12 pb-8 md:pt-20 md:pb-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Breadcrumbs items={[{ label: t('nav.platforms', 'Platforms') }]} />
          <div ref={heroRef} className="max-w-3xl opacity-0">
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-display font-bold tracking-tight text-zinc-900 dark:text-white">
              {t('platforms.title', 'Supported')}{' '}
              <span className="bg-clip-text text-transparent bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500">{t('platforms.titleGradient', 'Platforms')}</span>
            </h1>
            <p className="mt-4 text-lg text-zinc-600 dark:text-zinc-400 leading-relaxed">{t('platforms.subtitle', 'Six major advertising and analytics platforms, unified under a single MCP server. 180 tools for campaign management, tracking, and optimization.')}</p>
          </div>
        </div>
      </section>
      ...
      <section className="pb-20 md:pb-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {platforms.map((p) => <div key={p.slug} data-platform-card className="opacity-0"><PlatformCard platform={p} /></div>)}
          </div>
        </div>
      </section>
      ...
      <section className="py-20 md:py-32 bg-zinc-900 dark:bg-zinc-950 text-white relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-100px' }} transition={{ duration: 0.6 }} className="text-center">
            <h2 className="text-3xl md:text-4xl font-display font-bold tracking-tight">{t('cta.platformsTitle', 'Ready to Connect Your Platforms?')}</h2>
            <p className="mt-4 text-lg text-zinc-400 max-w-xl mx-auto">{t('cta.platformsSubtitle', 'One install to manage ads across every platform. Open source, forever free.')}</p>
